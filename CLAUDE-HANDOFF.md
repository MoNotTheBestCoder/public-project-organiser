...
**Setting up a session.** Session length, Focus block and Break step by five minutes and each carries a `list` of values worth jumping to, while any whole number in range can still be typed. Two details make that work together. The inputs validate through `minutesInRange`, not `checkValidity()`, because `step="5"` would otherwise make a typed 37 a `stepMismatch` and refuse it in silence, which is the same trap the modal duration field avoids with `novalidate`. And a number input anchors its step sequence at `min`, so `min="1"` with `step="5"` offers 1, 6, 11 and turns 25 into 26 on the first arrow press; `min` is therefore pinned at 0 and the real lower bound lives in `data-min`, which `minutesInRange` reads. Keep both in step if the ranges ever change.
