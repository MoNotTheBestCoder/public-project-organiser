...
- **Focus assignment picker:** Select a client, project, or task directly; adjacent browse arrows drill into children without selecting. Search across all three levels, including empty clients/projects. Keep the closed control to one line on both pages and floating windows, truncating long paths with a full title. The dropdown overlays content only after an explicit click; close/Escape returns focus to its summary. Use the existing chevrons, neutral surfaces, and a subtle selected tint. Anywhere else a client, project or task is chosen for focus time (the Add session and Edit details dialogs), and wherever a task's home is chosen (Add task, Edit task, Move), uses this same picker, never a native select; for a task's home it lists clients ("no project") and projects only. The Quick Add review keeps a select, because it must also offer clients and projects proposed in the same batch that do not exist yet; in a dialog its list opens in the flow instead of overlaying the fields below, and Escape closes the list, not the dialog. A destructive action in a form dialog sits at the left of the button row in the Delete style, taking a full-width row under Cancel and Save on a phone rather than squeezing three labels into one line.
...
- **Client layout:** Inside a client, its projects come first and the tasks with no project follow in a `No project` card, last in the same grid. It is built like a project card (twist, progress bar, counts, + Task), starts closed like one, and takes dropped tasks, which land on the client with no project. It is not a record, so it has no status, Edit, Delete, menu or drag handle, and its name is set in the secondary ink.
- **Steps:** A task can hold a checklist. A step reads as part of its task, a size down (13.5px) in the secondary ink with a 14px tick box, hanging off a 2px guide rule under the task, so a list of steps never passes for more tasks. A step is text and a tick and nothing else — giving one a status, a date or an assignment would make it a task and add a fourth level to the tree, which is the thing this feature exists to avoid. A task with no steps shows nothing at all: no chip, no placeholder, no affordance. One appears only once steps exist, reading `2 of 5 steps` and turning green at completion, with a small chevron that points right when closed and down when open; clicking it shows or hides the list, and closing always wins, even while the add-a-step field is open (anything typed there is kept as a step). Entry is through the row menu (`Break into steps`, then `Add a step`); typing Enter commits one and keeps the field open so a list goes in without a click per line. Ticking the last step never completes the task — a silent state change the user did not ask for is worse than an extra click.
...
- **Durations:** A length is one stepper box, never a pair of hours and minutes inputs. The value reads as a single run ("45 min", "1h 30m") centred on the whole box, not on the space beside the arrows, with the arrow column on the right. Arrows, Up/Down keys and mouse or trackpad scrolling over the box all step by five minutes (fifteen above an hour where a field allows long sessions), snapping to multiples; holding an arrow repeats and saves once on release. A scroll that starts on the page and sweeps across the box must not change it. Clicking the number opens an inline text box that accepts 90, 1:30, 1h 30m or 1.5h, so any whole minute in range can still be typed; out-of-range values clamp with a short hint, and unreadable ones say so rather than being refused in silence. All steppers share one height; the modal version is a little larger, but no field is enlarged for emphasis. On touch, the arrows move to either side as 48px targets. The real value lives in a hidden input, so modal forms keep `novalidate` and their own checks.
