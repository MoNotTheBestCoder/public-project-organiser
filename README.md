...
- Drag a task row onto a project or client, or onto the Unassigned target shown during dragging. Dragging a selected task moves the whole selection. Drag a project card onto a client to transfer the project and its tasks together. Clients cannot be dragged. Move/Edit dialogs remain available on touch and keyboard.
...
- **+ Add session** logs focused work you did without running the timer: a date, how long you focused, an optional client/project/task and note. Time is entered as hours and minutes — pick whole hours or five-minute steps from the dropdown, or type any number you like. The record it creates is identical to a timed one, so it exports and copies the same way.
- **Edit details** on any logged session can change its recorded time as well as its assignment and note. Editing the time replaces the timer's measurement, keeps the session's start so it stays in the same week, and updates your weekly total. Undo does not cover focus records, so the old figure is not recoverable.
