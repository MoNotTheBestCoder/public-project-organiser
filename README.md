...
- Use a task's **Move** button to choose another project, client, or Unassigned. Selecting a task reveals a compact toolbar for moving or deleting the selection. On desktop, checkboxes and row actions appear on hover or keyboard focus; touch controls remain visible.
...
- **+ Add session** logs focused work you did without running the timer: a date, the number of minutes, an optional client/project/task and note. The record it creates is identical to a timed one, so it exports and copies the same way.
